...
/// Test that timing-safe operations maintain consistency under load
#[test]
fn test_timing_consistency_under_load() {
    ...
    // Simulate concurrent timing-sensitive operations
    for thread_id in 0..4 {
        let results = results.clone();
        let handle = thread::spawn(move || {
            let mut local_analyzer = TimingAnalyzer::new();
            let data_a = vec![0x42u8; 32];
            let data_b = vec![if thread_id % 2 == 0 { 0x42u8 } else { 0x24u8 }; 32];

            for _ in 0..100 {
                let start = Instant::now();
                let _result = TimingSafe::bytes_equal(&data_a, &data_b);
                let duration = start.elapsed().as_nanos();
                local_analyzer.add_sample(duration);
            }

            let stats = local_analyzer.analyze();
            results.lock().unwrap().push((thread_id, stats));
        });
        handles.push(handle);
    }
    ...
    println!("Concurrent timing analysis results:");

    for (thread_id, stats) in all_results.iter() {
    ...
    let mean_times: Vec<f64> = all_results.iter().map(|(_, stats)| stats.mean).collect();
    let max_mean = mean_times.iter().copied().fold(f64::NEG_INFINITY, f64::max);
    let min_mean = mean_times.iter().copied().fold(f64::INFINITY, f64::min);
